...
\begin{document}
	...
	\section{Experimental results}
	{
		...
		\subsection{Cross-validation}
		{
			...
			\newline RVR lies in the family of bayesian methods for regression, it is therefore possible to derive a likelihood expression as in equation (\ref{eq::rvr_likelihood}). In this case, log-likelihood of the model therefore writes:
			\begin{equation}
			ln(L(\mathbf{t}{,X,w,\beta})) =  -\frac{1}{2}\beta^2 \sum_{i=1}{N}(t_n - y(x_n))^2
			\end{equation}
			where we recall that estimated parameter $\beta$ is supposed to represent the precision (inverse of the variance) of the noise affecting the output. Therefore, assuming known independent, normally distributed noise, the likelihood can naturally be related to the Mean Square Error (MSE) :
			\begin{equation}
				ln(L) = -\frac{N}{2\sigma^2}MSE
			\end{equation}
			 where $\sigma^2$ corresponds to the variance of the dataset's noise. Even though no likelihood expression can be derived for SVR, analogies between both methods lead to use previous equation to design the sparse regression evaluative metric in case of SVR.  
			...
			There are three hyperparameters to estimate for $\epsilon$-SVR. $\epsilon$ is supposed to represent the tube outside of which vectors are considered as support vectors, its optimal value should be close to the actual value for standard deviation of the output noise. The explored range [0.001, 10] seemed therefore adequate. $C$ penalizes for distances between points lying outside the $\epsilon$-tube and the border of the tube, it is hard to estimate beforehand the amount of order of the optimal value so that grid search was performed on the interval [0.1, 1000]. Finally, kernel width $\sigma$ represents the region of influence of the different support vectors. Given the spacing of inputs for the dataset, a good value should lie near 1 so that explored range was set to [0.001, 10].
			\newline For $\nu$-SVR, $\nu$ necessarily lies in the interval ]0,1[, the explored range, and represents a lower bound on the fraction of support vectors. Other considerations for kernel width $\sigma$ and penalization factor $C$ still hold.
						...
						Figures (\ref{fig::gridSearch_epsSVR_sinc}) and (\ref{fig::gridSearch_nuSVR_sinc}) display grid search results for both $\epsilon$-SVR and $\nu$-SVR in the form of heat-maps.  We can distinctively notice wells for both methods. It demonstrates the existence of parameters combinations that minimize our $BICSR$ metric. For $\epsilon$-SVR, the plateau corresponding to highest values for $\epsilon$ corresponds to models where all points lie inside the $\epsilon$-tube. Finally, as expected, optimal models for both methods are almost the same, since they share most of their support vectors, therefore showing the equivalence between both approaches. Both lead to a total of 14 support vectors.
				...
				\begin{figure}
					...
					\end{center}
				\end{figure}
			Figure (\ref{fig::gridSearch_RVR_sinc}) displays grid-search results for the RVR over kernel width $\sigma$, the only hyperparameter for this method. Found optimal value $1.28$ again confirms expectations.
			
			\begin{figure}
				\begin{center}
					\includegraphics[width=0.75\linewidth]{ROC_sinc}
					\caption{Evaluation of the tradeoff between raw performance ($MSE$) and sparsity (number of non-zero coefficients). Points correspond to the best hyperparameters for each method and each metric.}
					\label{fig::ROC_sinc}
				\end{center}
			\end{figure}
...
%			The $BIC$ criterion is a way to take model complexity into account when choosing an appropriate model. In our setting, the complexity is governed by the number of support vectors used and we are interested in the relationship between sparsity (the more support vectors the more complex the function can be) and performance (measured by the $MSE$). We analyze this tradeoff in figure \ref{fig::ROC_sinc} for the $sinc$ dataset. We can see that in terms of pure $MSE$ minimization both $\nu-SVR$ and $C-SVR$ perform slightly better than $RVR$ but at a very high cost in terms of sparsity since they have respectively $3$ and almost $5$ times more support vectors. For the hyperparameters that minimize the $BIC$ criterion, $SVR$ achieves a sparsity that is comparable to that of $RVR$ (although still there are still more support vectors) but this comes at the cost of a sharp rise of the $MSE$ of around $10\%$ for the $\nu-SVR$ and almost $50\%$ for the $C-SVR$. We can conclude that $RVR$ should be prefered as soon as the sparsity of the model is a concern.
			...
			\subsection{Real dataset}
			{
				...
				\begin{figure}
					...
					\end{center}
				\end{figure}
				Results of grid search over $\sigma$, the kernel width for Relevance Vectors Regression are displayed on figure (\ref{fig::airfoils_gridSearch_RVR_BIC}). Optimal kernel width therefore corresponds to $\sigma = 1.25$ leading to an MSE of 0.19. Number of retrieved relevance vectors is 94. 
