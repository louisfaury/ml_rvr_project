...
\begin{document}
	...
	\section{Results}
	{
		...
		\subsection{Cross-validation}
		{
			...
			For SVR, number of model parameters corresponds to the number of non-zero coefficients  $\alpha_i$ and $\alpha_i^*$ \textbf{Plutot $\hat{alpha}$ non? Et a quel point ça fait vraiment 2 free parameters sachant que c'est plutot leur }. $k$ is therefore equals to twice the number of support vectors  ($N_{SV}$)so that:
			\begin{equation}
			...
			Figures (\ref{fig::gridSearch_epsSVR_sinc}) and (\ref{fig::gridSearch_nuSVR_sinc}) display grid search results for both $\epsilon$-SVR and $\nu$-SVR in the form of a heat-map. As expected (\textbf{Are they the ones of figures 3 and 4?})both selected models are almost the same therefore showing the equivalence between both approaches. Both lead to a total of 14 support vectors.\\
