...
\begin{document}
	...
	\section{Results}
	{
		...
		\subsection{Cross-validation}
		{
			\subsubsection{Artificial dataset}
			{
			To assess best hyper-parameters for all three algorithms ($\epsilon$-SVR, $\nu$-SVR and RVR), grid search was performed using 50-fold cross validations. Chosen training test ratio was set to 0.75. Chosen metric to evaluate performance of the algorithms is the BIC (Bayesian Information Criterion). Among the properties of such metric is the ability to evaluate a compromise between complexity and as well as likelihood of the different models.
			General formula for BIC is:
			\begin{equation}
				BIC = -2*ln(L) + kln(M)
			\end{equation}
			where $L$ corresponds to the likelihood of the model, $k$ is the number of parameters of the model and $M$ is the number of datapoints. In the case of RVR and SVR, number of parameters $k$ is directly proportional to the number  of relevant vectors and respectively support vectors.
			}
		}
	}
	\section{Discussion}
