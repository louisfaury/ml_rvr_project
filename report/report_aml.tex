...
\begin{document}
	...
	\section{Theoretical background}
	{
		\subsection{Support Vector machine for Regression}
		{
			...
			\end{equation}
			To obtain sparse solution, the quadratic term is replaced by an $\eps$-\emph{insensitive error function} (see \cite{vapnik1995nature}) denoted $E_\eps(\cdot)$ with : 
			\begin{equation}
				\label{eq::loss}
				E_\eps(y(x)-t) = \left\{
								\begin{aligned}
			...
			\begin{equation}
				J(w) = C\sum_{n=1}^N E_\eps(w^T\phi(x_n)-t_n) + \frac{1}{2}\lVert w \rVert ^2
			\end{equation}
			where $C$ is a regularization parameter. 
			\newline 
			As for the SVM, one can introduce \emph{slack variables} in order to transform this optimization program into a Quadratic Programming (QP) problem (quadratic objective, linear constraints). Once the problem solved, predictions are made using : 
			\begin{equation}
			...
			As for the SVM, one can adopt a $\nu$-SVR formulation to have a lower-bound control on the number of retained support vectors. 
		}
