...
\begin{document}
	...
	\section{Experimental results}
	{
		...
		\subsection{Cross-validation}
		{
			...
			\newline In the case of RVR and SVR, the number of parameters $k$ is directly proportional to the number of relevant vectors (respectively support vectors). Assuming independent, normally distributed noise, the likelihood can directly be related to the Mean Square Error (MSE) :
			\begin{equation}
				ln(L) = -\frac{N}{2\sigma^2}MSE
			\end{equation}
			where $\sigma^2$ corresponds to the variance of the dataset's noise. Therefore a reasonable metric to measure the goodness of a sparse regression model would be 
			\begin{equation}
			...
			\subsubsection{Grid-search cross-validation}
			{
				...
				In order to find the best parameters, we started with a very wide range of values (between 10^{-3} and 10^3 for every}) and a coarse granularity (10 points for every parameter) to locate the interesting regions. This allowed us to guess more precisely where the optimal region was. Then, we ran a second grid-search with 20 points for each dimensions around the best point for the previous one. We chose a range that roughly correspond to the interval between 2 neighboring points in the previous grid search. 
				\begin{figure}
					\begin{center}
			...
			Figures (\ref{fig::gridSearch_epsSVR_sinc}) and (\ref{fig::gridSearch_nuSVR_sinc}) display grid search results for both $\epsilon$-SVR and $\nu$-SVR in the form of heat-maps. If we fix $C$, there is a small region of low values for our objective function that then increases as we move further away. Changing the value of $C$ only changes the size of the region and its value but not the global behaviour, which suggests that this parameter has little influence on the result as long as values are not too far from the optimal. This leads us to believe that there is a global minimum for our objective function, that this minimum is captured by our grid and therefore that the result of our grid search is reliable. The form of the regions is different between $\epsilon$-SVR and $\nu$-SVR but comments are the same. 
			 ...
			 As expected both selected models are almost the same therefore showing the equivalence between both approaches. Both lead to a total of 14 support vectors.
				...
				\begin{figure}
					...
					\end{center}
				\end{figure}
			Figure (\ref{fig::gridSearch_RVR_sinc}) displays grid-search results for the RVR over kernel width $\sigma$, the only hyperparameter for this method. 
			
			Since RVR only requires to set $1$ parameter, the grid search is even easier and the optimal parameter is located at what appears to be a global minimum for the objective function as shown in figure~\ref{fig::gridSearch_RVR_sinc}.
